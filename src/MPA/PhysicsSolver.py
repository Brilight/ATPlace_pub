...
class Thermal_solver():
    ...
    def set_pos(self, power, pos, size):
...
class ATSim_solver(Thermal_solver):
    ...
    @staticmethod
    def execute_command(cmd, cmd_name, print_log=1, timeout_seconds=1000, cwd=None):
        try:
            if print_log:
                ...
                if result.returncode != 0:
                    print(f"Error: {cmd_name} failed with return code {result.returncode}")
            ...
            print(f"Error: {cmd_name} timed out after {timeout_seconds} seconds")
            print(f"Command: {' '.join(cmd)}")
            return False
        except FileNotFoundError:
            print(f"Error: Command not found - {cmd_name}: {cmd}")
            return False
        except Exception as e:
            print(f"Error: Failed to run {cmd_name}: {e}")
            return False
    ...
    def gen_stack(self, dir_name):
        ...
        ET.SubElement(package, "MaterialLib", attrib={
            "File": "../ATSim/material.config"
        })
        ET.SubElement(package, "PowerLib", attrib={
            "File": f"../ATSim/{dir_name}/powerlib.yml"
        })
        ET.SubElement(package, "TempData", attrib={
            "File": f"../ATSim/{dir_name}/raw/steadytemp.txt"
        })
        ...
        for chiplet_idx in range(self.num_chiplets):
            ...
            for layer_name in self.layers['component2'].keys():
                ...
                if "act" in layer_name.lower():
                    ET.SubElement(layer, "Floorplan", attrib={
                        "Type": "Macro", 
                        "File": f"../ATSim/{dir_name}/flp/Chiplet_{chiplet_idx}_flp.csv"
                    })
                    ET.SubElement(layer, "Power", attrib={
                        "File": f"../ATSim/{dir_name}/power/Chiplet_{chiplet_idx}_power.csv"
                    })
    ...
    def run(self, dir_name, default=1, time_limit=2000):
        os.system('rm -rf ' + os.path.join(self.path, f"{dir_name}"))
        ...
        cmd1 = [
            "python", "/home/qpwang/ATSim3D/srcv2/ATSim3_5D.py",
            "-xml", xml_file,
            "-config", sim_config,
            "--output_path", output_dir
        ]
        success1 = self.execute_command(cmd1, "ATSim3_5D.py", not default, time_limit, self.path)
        if not success1:
            raise RuntimeError("ATSim3_5D.py failed.")
        return
    
    def getres(self, dir_name):

        xi_temp = np.linspace(0, self.intp_width / 1e3, self.num_grid_x)
        yi_temp = np.linspace(0, self.intp_height / 1e3, self.num_grid_y)
        Xi_temp, Yi_temp = np.meshgrid(xi_temp, yi_temp, indexing='xy')  # [Ny, Nx]

        def interpolate_scattered(x, y, val, xi, yi, method='cubic'):
            points = np.stack((x, y), axis=-1)  # [N, 2]
            zi = griddata(points, val, (xi, yi), method=method)

            nan_mask = np.isnan(zi)
            if np.any(nan_mask):
                print(f"NaN in {method} interpolation: {nan_mask.sum()}/{nan_mask.size}")
                zi_linear = griddata(points, val, (xi, yi), method='linear')
                zi[nan_mask] = zi_linear[nan_mask]
                nan_mask = np.isnan(zi)
                if np.any(nan_mask):
                    zi_nearest = griddata(points, val, (xi, yi), method='nearest')
                    zi[nan_mask] = zi_nearest[nan_mask]
            return zi

        res_temp = np.loadtxt(os.path.join(self.path, f"{dir_name}/raw/steadytemp.txt"))
        z_min = self.z_origin
        z_max = self.z_origin + 1e-6
        plane_idx = (res_temp[:, 4] >= z_min) & (res_temp[:, 4] <= z_max)

        if not np.any(plane_idx):
            raise ValueError("No temperature data in target z-plane.")

        x_temp = (res_temp[plane_idx, 0] + res_temp[plane_idx, 1]) / 2  # [N,]
        y_temp = (res_temp[plane_idx, 2] + res_temp[plane_idx, 3]) / 2  # [N,]
        val_temp = res_temp[plane_idx, -1]                              # [N,]
        temp_map = interpolate_scattered(x_temp, y_temp, val_temp, Xi_temp, Yi_temp)

        return {
            'temperature': np.transpose(temp_map.astype(np.float32), (1,0))
        }
            
class Warpage_solver(ATSim_solver):
    def __init__(self, system):
        super(Warpage_solver, self).__init__(system)
    ...
    def run(self, dir_name, default=1, time_limit=2000):
        ...
        with open(xml_file, 'w', encoding='utf-8') as f:
            f.write(pretty_xml)
        self.gen_flp_and_power(dir_name)
        print(f"Init Input time {time.time()-t1:.3f}")
        cmd1 = [
            "python", "/home/qpwang/ATSim3D/srcv2/ATSim3_5D.py",
            "-xml", xml_file,
            "-config", sim_config,
        ...
        if not success1:
            ...
            print(f"Thermal solve time {time.time()-t1:.3f}")

        steady_file = os.path.join(self.path, f"{dir_name}/raw/steadytemp.txt")
        if not os.path.exists(steady_file):
            print(f"Temp solution not exists at {steady_file}")
        
        def run_remote(target_ip, target_user, target_dir):
            remote_dir = os.path.join(target_dir, dir_name)
            remote_log = f"{remote_dir}/remote_run.log"

            try:
                subprocess.run([
                    "scp", "-r", os.path.join(self.path, dir_name),
                    f"{target_user}@{target_ip}:{target_dir}/"
                ], check=True)
                print(f"Transfer complete from {dir_name} to {target_dir}")
            except Exception as e:
            ...
            remote_cmd = f"""
                ...
                conda activate atsim && \
                cd {target_dir} && \
                mpirun -np 16 python ~/ATSim3D/srcv1/Main.py \
                    -xml {dir_name}/package.xml \
                    -config ./sim.config \
                    --output_path {dir_name}/ > {remote_log} 2>&1
            """
            ...
            def retrieve_log():
                ...
                subprocess.run([
                        "scp",
                        f"{target_user}@{target_ip}:{remote_log}",
                        os.path.join(self.path, f"{dir_name}/raw")
                    ], check=False)
                print(f"Warpage solve time {time.time()-t1:.3f}")
                with open(os.path.join(self.path, f"{dir_name}/raw", "remote_run.log"), 
                "r") as f:
                    print(f.read())

            try:
                result = subprocess.run(
                    ["ssh", f"{target_user}@{target_ip}", remote_cmd],
                ...
                print("Remote command timeout")
                try:
                    retrieve_log()
                except Exception as e:
                    print(f"Fetch log after timeout failed: {e}")
                return False
            except Exception as e:
            ...
            try:
                subprocess.run([
                    "scp", 
                    f"{target_user}@{target_ip}:{os.path.join(remote_dir, 'raw/disp_z.txt')}",
                    os.path.join(self.path, f"{dir_name}/raw/")
                ], check=True)
            except Exception as e:
                print(f"Fetch result failed: {e}")
                return False

            print("Remote run completed and result fetched.")
            return True

        success2 = run_remote(
            "10.129.166.134", "qipanwang", "/home/qipanwang/ATPlace2.5D/ATSim/"
        )
        return
    ...
    def getres(self, dir_name):
        ...
        def interpolate_regular(x_coords, y_coords, val, xi, yi):
            ...
            zi = interpolator(query_points).reshape(xi.shape)
            return zi
        ...
        temp_map = interpolate_scattered(x_temp, y_temp, val_temp, Xi_temp, Yi_temp)
        # temp_map.shape = [num_grid_y, num_grid_x]

        res_disp = np.loadtxt(os.path.join(self.path, f"{dir_name}/raw/disp_z.txt"))
        val_disp = res_disp.reshape(100, 100)  # [100, 100], row-major
        x_disp_coords = np.linspace(0, self.intp_width / 1e3, 100)
        y_disp_coords = np.linspace(0, self.intp_height / 1e3, 100)
        warpage = interpolate_regular(x_disp_coords, y_disp_coords, val_disp, Xi_temp, Yi_temp)
        # warpage.shape = [num_grid_y, num_grid_x]

        return {
            'temperature': np.transpose(temp_map.astype(np.float32), (1,0)), 
            'warpage': np.transpose(warpage.astype(np.float32), (1,0)) 
        }
